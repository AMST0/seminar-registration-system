  ...
  return (
    <div className="min-h-screen bg-stone-100 flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex justify-between items-center h-14">
            <div className="flex items-center gap-4">
              <div className="w-8 h-8 bg-rose-500 flex items-center justify-center">
              ...
              <div>
                <h1 className="text-sm font-bold tracking-wide uppercase">Seminer Yönetimi</h1>
              </div>
            </div>
            <div className="flex items-center gap-6">
              <button
                onClick={() => signOut({ callbackUrl: '/admin/login' })}
                className="inline-flex items-center gap-2 px-4 py-1.5 text-xs font-medium text-stone-400 
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 lg:px-8 py-8 w-full">
        {/* Stats - Sharp Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-1 mb-8">
          <div className="bg-white p-6 border-l-4 border-rose-500">
            <p className="text-[10px] text-stone-400 font-medium uppercase tracking-widest mb-1">Toplam Seminer</p>
            <p className="text-3xl font-bold text-stone-900">{stats.totalSeminars}</p>
          </div>
          
          <div className="bg-white p-6 border-l-4 border-amber-500">
            <p className="text-[10px] text-stone-400 font-medium uppercase tracking-widest mb-1">Aktif Seminer</p>
            <p className="text-3xl font-bold text-stone-900">{stats.activeSeminars}</p>
          </div>
          
          <div className="bg-white p-6 border-l-4 border-stone-500">
            <p className="text-[10px] text-stone-400 font-medium uppercase tracking-widest mb-1">Toplam Kayıt</p>
            <p className="text-3xl font-bold text-stone-900">{stats.totalRegistrations}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h2 className="text-lg font-bold text-stone-900 uppercase tracking-wide">Seminerler</h2>
            <p className="text-xs text-stone-500">Tüm seminerleri yönetin</p>
          </div>
          <button
        ...
        {stats.activeSeminars >= 10 && (
          <div className="flex items-center gap-3 p-4 bg-amber-50 border-l-4 border-amber-500 mb-6">
            <svg className="w-5 h-5 text-amber-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} 
                    d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <p className="text-sm text-amber-800 font-medium">Maksimum 10 aktif seminer sınırına ulaştınız.</p>
          </div>
        )}

        {/* Seminars List - Sharp Design */}
        <div className="space-y-1">
          {seminars.map((seminar) => (
            <div key={seminar.id} className="bg-white border-l-4 border-transparent hover:border-rose-500 transition-colors">
              <div className="p-6">
                <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-base font-bold text-stone-900 uppercase tracking-wide truncate">{seminar.title}</h3>
                    ...
                    {seminar.description && (
                      <p className="text-sm text-stone-500 mb-3 line-clamp-2">{seminar.description}</p>
                    )}
                    ...
                    <div className="mt-3">
                      <code className="text-[10px] bg-stone-100 text-stone-600 px-2 py-1 font-mono tracking-wide">
                        /s/{seminar.slug}
                      </code>
                    </div>
                  </div>
              ...
              {showRegistrations === seminar.id && (
                <div className="border-t border-stone-200 bg-stone-50 p-6">
                  <h4 className="font-bold text-xs uppercase tracking-wider text-stone-900 mb-4">Kayıtlı Katılımcılar</h4>
                  {seminar.registrations.length === 0 ? (
                    <p className="text-stone-500 text-sm">Henüz kayıt yok.</p>
                  ) : (
      ...
      {/* Footer */}
      <footer className="bg-stone-900 text-stone-400 py-4 mt-auto">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex justify-between items-center">
          <p className="text-[10px] uppercase tracking-widest">© 2026 Seminer Yönetim Sistemi</p>
          <a
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
        <div className="bg-white max-w-lg w-full max-h-[90vh] overflow-hidden shadow-2xl">
          {/* Header */}
          <div className="p-6 border-b border-stone-200 bg-stone-900 text-white">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={onClose}
                className="w-8 h-8 bg-stone-800 hover:bg-stone-700 flex items-center justify-center transition-colors"
              >
                <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
